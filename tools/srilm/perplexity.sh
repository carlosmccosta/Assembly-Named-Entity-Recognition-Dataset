...
ngram -lm "${output_file}_train_${ngram_model}-gram_smoothing-modified-kneser-ney.lm" -order ${ngram_model} -ppl "${text_file}_test.txt" -debug ${debug} -memuse > "${output_file}_test_${ngram_model}-gram_smoothing-modified-kneser-ney.ppl"
